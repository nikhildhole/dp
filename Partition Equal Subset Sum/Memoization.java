public class PartitionEqualSum {
    ...
    public static boolean canPartition(int[] nums) {
        ...
        return cP(nums, sum/2, nums.length, dp) == 1? false : true;
    }

    public static int cP(int[] nums, int sum, int i, int[][] dp){
        if(sum == 0){
            return 2;
        }
        if(i == 0){
        ...
        if(sum < nums[i-1]){
            dp[i][sum]=  cP(nums, sum, i-1, dp);
        }else{
            dp[i][sum]= cP(nums, sum, i-1, dp) == 2 ? 2 : cP(nums, sum - nums[i-1], i-1, dp);
        }
        return dp[i][sum];
    }
