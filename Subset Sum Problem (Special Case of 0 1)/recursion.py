def iss(nums, target, n, dp):
    ...
    if(target < nums[n-1]):
        dp[n][target] = iss(nums=nums, target=target, n=n-1, dp=dp)
    else:
        dp[n][target] = iss(nums=nums, target=target, n=n-1, dp=dp) or iss(nums=nums, target=target-nums[n-1], n=n-1, dp=dp)
