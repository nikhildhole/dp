function csws(nums, target, sum, i, n){
    if(n == i){
        return target === sum ? 1 : 0
    }

    var exlcude = csws(nums, target, sum, i + 1,  n)

    var inlcude = csws(nums, target, sum + nums[i], i + 1,  n)

    return exlcude + inlcude
}

function countSubsetsWithSum(nums, target) {
    // TODO: Implement this function
    return csws(nums, target, 0, 0, nums.length)
}
